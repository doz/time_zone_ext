...
describe TimeZoneExt do
  ...
  it "parses time with timezone specified by name" do
    time = Time.zone.strptime("2012-06-02 00:00 UTC", "%Y-%m-%d %H:%M %Z")
    expect(time.to_s).to eq("2012-06-01 19:00:00 -0500")
  end

  it "parses time with timezone specified by offset" do
    time = Time.zone.strptime("2012-06-02 00:00 +0100", "%Y-%m-%d %H:%M %z")
    expect(time.to_s).to eq("2012-06-01 18:00:00 -0500")
  end
  ...
  it "parses time with named time zone" do
    Time.zone = "Moscow"
    time = Time.zone.strptime("2012-06-02 00:00", "%Y-%m-%d %H:%M")
    expect(time.to_s).to eq("2012-06-02 00:00:00 +0400")
  end
end
